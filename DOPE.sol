...
Smart-contract developed by snipe.finance: info@snipe.finance - t.me/SnipeFinance
...
// File: contracts/DOPEDividendPayingToken.sol
...
contract DOPE is ERC20, Ownable {
    ...
    DOPEDividendTracker public dividendTracker;
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "DOPE: The dividend tracker already has that address");

        DOPEDividendTracker newDividendTracker = DOPEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DOPE: The new dividend tracker must be owned by the DOPE token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "DOPE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "DOPE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "DOPE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DOPE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DOPE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DOPE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DOPE: This account cannot send tokens until trading is enabled");
        }
...
contract DOPEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("DOPE_Dividend_Tracker", "DOPE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 888888888 * (10**18); //must hold 888888888+ tokens
    } 
      //Smart-contract developed by snipe.finance: info@snipe.finance - t.me/SnipeFinance
    function updateMinimumBalanceForDividends(uint256 newAmountNoDecimials) external onlyOwner{
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "DOPE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DOPE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DOPE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DOPE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DOPE_Dividend_Tracker: Cannot update claimWait to same value");
